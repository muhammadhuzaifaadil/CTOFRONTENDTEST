...
type ReportTabKey =
 ...
 const handleBuyerExport = async () => {
  try {
    const response = await apiClient.get('/admin/buyers/export', {
 ...
 const handleSellerExport = async () => {
  try {
    const response = await apiClient.get('/admin/sellers/export', {
      responseType: 'blob', // 👈 REQUIRED
    });
...
const Reports: React.FC = () => {
  ...
  useEffect(() => {
    apiClient.get('/admin/ReportsOverview')
      .then((response) => {
...
const completedProjects = overview?.lineChart?.completed ?? [];
  ...
  const statusPieData = [
    { id: 0, value: 65, label: "Completed" },
    { id: 1, value: 30, label: "In Progress" },
    { id: 2, value: 4, label: "Cancelled" },
    { id: 3, value: 1, label: "Disputed" },
  ]
const PROJECT_CATEGORIES = ["ERP", "Web Development", "App Development"];
...
const BuyerActivitySection: React.FC = () => {
  ...
  useEffect(() => {
    apiClient.get('/admin/BuyerReports')
      .then((response) => {
...
const SellerPerformanceSection: React.FC = () => {
  ...
  useEffect(() => {
    apiClient.get('/admin/SellerReports')
      .then((response) => {
  ...
  const summaryCards = [
    {
      ...
      value: `${sellerReport?.projectsAccepted}%`,
  ...
  
    {
      ...
      value: `${sellerReport?.avgSellerRating ?? "4.5"}`,
      // change: "+0.2 vs last period",
      // trend: "up" as const,
    },
    {
      title: "Rejection Rate",
      value: `${sellerReport?.rejectionRate}%`,
